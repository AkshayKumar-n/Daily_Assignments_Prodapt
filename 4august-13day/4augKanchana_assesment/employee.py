...
headerContent=["Name","designation","employeeid","salary","phonenumber","address","pincode"]
...
while(1):
    ...
    if(choice==2):
        print(json.maps(employee_dict.maps))

    if(choice==3):
        sal=int(input("Enter the minimum salary:"))
        for i in employee_dict.maps:
            if int(i['salary'])>=sal:
                print(i)
    if choice==4:
        with open("employee.csv","w+",encoding="UTF8",newline='')as e:
            writer=csv.DictWriter(e,fieldnames=headerContent)
            writer.writeheader()
            writer.writerows(employee_dict.maps)
    if(choice==4):
        break
